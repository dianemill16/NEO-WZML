...
from bot.helper.ext_utils.bot_utils import new_task, arg_parser, get_size_bytes
    ...
    async with rss_dict_lock:
        return next(
            (
                ...
                if isinstance(feeds, dict) and title in feeds
            ),
            (False, False),
        )
...
async def refresh_shared_tasks_timestamps():
    if database._return or Config.UNIVERSAL_MAX_TASKS == 0:
        return
    try:
        for task_id, task in task_dict.items():
            try:
                await database.refresh_task_timestamp(task_id, TgClient.ID)
                await database.refresh_universal_task_lock(
                    task.listener.user_id, task_id, TgClient.ID
                )
            except Exception as e:
                LOGGER.debug(f"Failed to refresh timestamp for task {task_id}: {e}")
    except Exception as e:
        LOGGER.error(f"Error in shared tasks timestamp refresh: {e}")


def add_shared_tasks_refresh_job():
    if Config.UNIVERSAL_MAX_TASKS > 0:
        scheduler.add_job(
            refresh_shared_tasks_timestamps,
            trigger="interval",
            hours=1,
            id="shared_tasks_refresh",
            name="SharedTasksRefresh",
            misfire_grace_time=300,
            max_instances=1,
            next_run_time=datetime.now() + timedelta(minutes=10),
            replace_existing=True,
        )
        LOGGER.info("Shared tasks timestamp refresh job scheduled (every 1 hour)")


async def reconcile_universal_locks():
    try:
        cleaned = await database.reconcile_universal_task_locks()
        LOGGER.debug(
            f"Universal locks reconciliation finished, cleaned={cleaned}"
        )
    except Exception as e:
        LOGGER.error(f"Error in universal_task_locks reconciliation: {e}")


def add_universal_locks_reconcile_job():
    if database._return:
        return
    scheduler.add_job(
        reconcile_universal_locks,
        trigger="interval",
        minutes=30,
        id="universal_locks_reconcile",
        name="UniversalLocksReconcile",
        misfire_grace_time=600,
        max_instances=1,
        next_run_time=datetime.now() + timedelta(minutes=15),
        replace_existing=True,
    )
    LOGGER.info("Universal task locks reconciliation job scheduled (every 30 min)")


async def bot_heartbeat_loop():
    bot_id = TgClient.ID
    LOGGER.info(f"Starting heartbeat loop for bot {bot_id}")

    while True:
        try:
            async with task_dict_lock:
                active_task_ids = list(task_dict.keys())

            await database.update_active_tasks(bot_id, active_task_ids)
            LOGGER.debug(
                f"Heartbeat: Bot {bot_id} has {len(active_task_ids)} active tasks"
            )

        except Exception as e:
            LOGGER.error(f"Heartbeat update failed for bot {bot_id}: {e}")
...
add_universal_locks_reconcile_job()
scheduler.start()
